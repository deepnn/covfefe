...
Your code doesn't work, and you have determined that the issue lies with coffee? Follow these steps to report a bug.

1. Your bug may already be fixed. Make sure to update to the current coffee master branch, as well as the latest Theano/TensorFlow/Caffe master branch.
To easily update Theano: `pip install git+git://github.com/Theano/Theano.git --upgrade`

2. Search for similar issues. Make sure to delete `is:open` on the issue search to find solved tickets as well. It's possible somebody has encountered this bug already. Also remember to check out coffee' [FAQ](http://deepnn.ml/coffee/faq/). Still having a problem? Open an issue on Github to let us know.

3. Make sure you provide us with useful information about your configuration: what OS are you using? What coffee framework are you using? Are you running on GPU? If so, what is your version of Cuda, of cuDNN? What is your GPU?
...
You can also use Github issues to request features you would like to see in coffee, or changes in the coffee API.

1. Provide a clear and detailed explanation of the feature you want and why it's important to add. Keep in mind that we want features that will be useful to the majority of our users and not just a small subset. If you're just targeting a minority of users, consider writing an add-on library for coffee. It is crucial for coffee to avoid bloating the API and codebase.
...
5. Run our test suite locally. It's easy: from the coffee folder, simply run: `py.test tests/`.
...
6. Make sure all tests are passing:
  - with the Theano framework, on Python 2.7 and Python 3.5
  - with the TensorFlow framework, on Python 2.7
  - with the Caffe framework, on python 2.7
...
7. We use PEP8 syntax conventions, but we aren't dogmatic when it comes to line length. Make sure your lines stay reasonably sized, though. To make your life easier, we recommend running a PEP8 linter:
  ...
  - You can automatically fix some PEP8 error by running: `autopep8 -i --select <errors> <FILENAME>` for example: `autopep8 -i --select E128 tests/coffee/frameworks/test_frameworks.py`
...
Even if you don't contribute to the coffee source code, if you have an application of coffee that is concise and powerful, please consider adding it to our collection of examples. [Existing examples](https://github.com/deepnn/coffee/tree/master/examples) show idiomatic coffee code: make sure to keep your own script in the same spirit.
